...
In `r capitalize_region(region)` there are `r nrow(l)` between-zone flows that a) start and end in `r capitalize_region(region)`, b) have a straight-line (Euclidean) distance of less than `r mdist`km and a fast-route distance less than 30km, and c) contain more than `r mflow` commuters (by any mode, counting commuters in both directions).   These `r nrow(l)` between-zone flows are visualised as **Straight Lines**, **Routes** (fast and quiet) and the **Route Network** on the interactive map, and account for `r round(sum(l$all) / sum(zones$all) * 100)`% of all commuters living in `r capitalize_region(region)`.

Between-zone flows exclude **within-zone travel**, when the zone of origin is the same as the zone of destination. Within-zone travel is represented by red points on the map when the lines are shown, and accounts for `r round(sum(cents$all) / sum(zones$all) * 100)`% of commuters in `r capitalize_region(region)`. The between-zone flows visualised as lines and routes on the map also exclude commuters travelling outside `r capitalize_region(region)` and people with no fixed place of work.
...
<!--Can we please have here a table for the whole region per scenario, using area-based data? This would look like the following.  Cycling/walking/car drivers cells the percent is equal to the scenario level of cycling across all the zones, divided by 'all'.  In the "all other modes" cell, the percent is equal to 100% minus the first 3 rows. I suggest one decimal place.
  ...
  Ebikes
-->
<!-- **Table 1:** Mode split among commuters living in `r capitalize_region(region)` across scenarios
-->
...
| Ebikes  | `r paste((b <- round(100 * sum(zones$ebike_slc) / sum(zones$all) , 1)), '%')` | `r paste((f <- round(100 * sum(zones$ebike_slw) / sum(zones$all) , 1)), '%')`|`r paste((cd <- round(100 * sum(zones$ebike_sld) / sum(zones$all) , 1)), '%')` | `r paste((100 - b - f - cd), '%')` |
...
<!--
**Table 2:** Mortality and carbon impacts of scenarios in `r capitalize_region(region)`, relative to baseline and to a ‘no cyclists’ counterfactual.  Note that 'MTCO2e' stands for meg-tonnes of CO2-equivalent emissions
-->
...
| -------- |:----------:|:-----------------: |:----------:| :---------------:|
| Census 2011 |	'No Cyclists' |  `r (round(sum(zones$govtarget_sldeath_heat) - sum(zones$govtarget_sideath_heat) , 2))` | `r (round((sum(zones$govtarget_slvalue_heat) - sum(zones$govtarget_sivalue_heat)) / 1000000  , 2))` | `r (round( ( sum(zones$govtarget_slco2) - sum(zones$govtarget_sico2) ) / 1000))` |
| Government Target |	'No Cyclists' |  `r (round(sum(zones$govtarget_sldeath_heat) , 2))` | `r (round(sum(zones$govtarget_slvalue_heat) / 1000000  , 2))` | `r (round( sum(zones$govtarget_slco2) / 1000))` |
|  |	Census 2011 |  `r (round(sum(zones$govtarget_sideath_heat) , 2))` | `r (round(sum(zones$govtarget_sivalue_heat) / 1000000  , 2))` | `r (round( sum(zones$govtarget_sico2) / 1000))` |
| Gender Equity |	'No Cyclists' |  `r (round(sum(zones$gendereq_sldeath_heat) , 2))` | `r (round(sum(zones$gendereq_slvalue_heat) / 1000000  , 2))` | `r (round( sum(zones$gendereq_slco2) / 1000))` | 
|  |	Census 2011 |  `r (round(sum(zones$gendereq_sideath_heat) , 2))` | `r (round(sum(zones$gendereq_sivalue_heat) / 1000000  , 2))` | `r (round( sum(zones$gendereq_sico2) / 1000))` |
| Go Dutch |	'No Cyclists' |  `r (round(sum(zones$dutch_sldeath_heat) , 2))` | `r (round(sum(zones$dutch_slvalue_heat) / 1000000  , 2))` | `r (round( sum(zones$dutch_slco2) / 1000))` |
|  |	Census 2011 |  `r (round(sum(zones$dutch_sideath_heat) , 2))` | `r (round(sum(zones$dutch_sivalue_heat) / 1000000  , 2))` | `r (round( sum(zones$dutch_sico2) / 1000))` |
| Ebikes |	'No Cyclists' |  `r (round(sum(zones$ebike_sldeath_heat) , 2))` | `r (round(sum(zones$ebike_slvalue_heat) / 1000000  , 2))` | `r (round( sum(zones$ebike_slco2) / 1000))` |
|  |	Census 2011 |  `r (round(sum(zones$ebike_sideath_heat) , 2))` | `r (round(sum(zones$ebike_sivalue_heat) / 1000000  , 2))` | `r (round( sum(zones$ebike_sico2) / 1000))` |
