...
# Read-in the data
root <- "../.."
od_attributes <- read_csv(file.path(root, path_outputs_regional, purpose, geography, region, "od_attributes.csv"))
c <- readRDS(file.path(root, path_outputs_regional, purpose, geography, region, "c.Rds"))
z <- readRDS(file.path(root, path_outputs_regional, purpose, geography, region, "z.Rds"))
l <- readRDS(file.path(root, path_outputs_regional, purpose, geography, region, "l.Rds"))
rf <- readRDS(file.path(root, path_outputs_regional, purpose, geography, region, "rf.Rds"))
...
The **Route Network (LSOA)** is built from all between-zone LSOA fast routes that a) start *or* end in `r get_pretty_region_name(region)`, and b) have a fast route distance <20km. For more details please see User Manual C.
...
The proportion of commuters who cycled as their main mode of travel to work baseline (2011 Census) data in `r get_pretty_region_name(region)` was 
...
Table 1 summarises the proportion of commuters walking, driving and using other modes under each scenario in `r get_pretty_region_name(region)`, based on aggregating across all residents in all the LSOA zones.
