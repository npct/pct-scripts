...
# Read-in the data
if(!exists("region")) region <- "greater-manchester"
...
dfsp$scenario <-
  factor(dfsp$scenario, levels = levels(dfsp$scenario)[c(1, 3, 2, 4, 5, 6, 7)])
levels(dfsp$scenario)[1] <- c("Total no. commuters")
levels(dfsp$scenario)[6] <- c("Cyclists in Near Market")
levels(dfsp$scenario)[7] <- c("Cyclists in Census 2011")
...
| Near Market | `r paste(sprintf('%.1f', (b <- round(100 * sum(zones$nearmkt_slc) / sum(zones$all) , 1))), '%')` | `r paste(sprintf('%.1f', (f <- round(100 * sum(zones$nearmkt_slw) / sum(zones$all) , 1))), '%')`|`r paste(sprintf('%.1f', (cd <- round(100 * sum(zones$nearmkt_sld) / sum(zones$all) , 1))), '%')` | `r paste(sprintf('%.1f', (100 - b - f - cd)), '%')` |
...
| Near Market |	'No Cyclists' |  `r (sprintf('%.2f', round(sum(zones$nearmkt_sldeath_heat) , 2)))` | `r (sprintf('%.2f', round(sum(zones$nearmkt_slvalue_heat) / 1000000  , 2)))` | `r (round( sum(zones$nearmkt_slco2) / 1000))` |
|  |	Census 2011 |  `r (sprintf('%.2f', round(sum(zones$nearmkt_sideath_heat) , 2)))` | `r (sprintf('%.2f', round(sum(zones$nearmkt_sivalue_heat) / 1000000  , 2)))` | `r (round( sum(zones$nearmkt_sico2) / 1000))` |
