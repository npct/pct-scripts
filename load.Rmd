...
buff_dist <- 7 # buffer (km) used to select additional zones (often zero = ok)
...
```{r, echo=FALSE}
l$slc <- -3.2941 + (-0.4659 * l$dist_fast) + (1.3430 * dist_fastsqrt) + (0.0062 * dist_fastsq) + (-31.9249 * ned_avslope) + (1.0989 * interact) + (-6.6162 * interactsqrt) + (2.4783 * dutch) + (-0.0706 * dutch*dist_fast) + (0.0286 * ebike) + (0.0902 * ebike*dist_fast) + (-0.0014 * ebike*dist_fastsq) 
...
zones@data[is.nan(zones$av_distance), c("av_distance", "cirquity", "distq_f")] <- NA
...
geojson_write( ms_simplify(l, keep = 0.1), file = file.path(pct_data, region, "l"))
saveRDS(rf, file.path(pct_data, region, "rf.Rds"))
geojson_write( ms_simplify(rf, keep = 0.1), file = file.path(pct_data, region, "rf"))
saveRDS(rq, file.path(pct_data, region, "rq.Rds"))
geojson_write( ms_simplify(rq, keep = 0.1), file = file.path(pct_data, region, "rq"))
saveRDS(rnet, file.path(pct_data, region, "rnet.Rds"))
geojson_write( ms_simplify(rnet, keep= 0.1), file = file.path(pct_data, region, "rnet"))
