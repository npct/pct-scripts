...
if(!dir.exists(file.path(path_outputs_national, purpose_public))) { dir.create(file.path(path_outputs_national, purpose_public)) }
if(!dir.exists(file.path(path_outputs_national, purpose_public, geography))) { dir.create(file.path(path_outputs_national, purpose_public, geography)) }
...
z_all_attributes_public <- read_csv(file.path(path_temp_scenario, purpose, geography, "z_all_attributes_public_unrounded.csv"))
d_all_attributes_public <- read_csv(file.path(path_temp_scenario, purpose, geography, "d_all_attributes_public_unrounded.csv"))
...
z_all_attributes_public <- z_all_attributes_public[order(z_all_attributes_public$geo_name),] # A bodge: sort so as not to have at the top an LSOA that has 0 cars in, as this messes up rounding.  Correct 2 lines later
z_all_attributes_public <- as.data.frame(apply(z_all_attributes_public, c(2), round_df), stringsAsFactors = F)
z_all_attributes_public <- z_all_attributes_public[order(z_all_attributes_public$geo_code),] 
z_all_attributes_public <- z_all_attributes_public[z_codebook$`Variable name`]
write_csv(z_all_attributes_public,  file.path(path_outputs_national, purpose_public, geography, "z_all_attributes.csv"))
...
d_all_attributes_public <- as.data.frame(apply(d_all_attributes_public, c(2), round_df), stringsAsFactors = F)
d_all_attributes_public <- d_all_attributes_public[d_codebook$`Variable name`]
write_csv(d_all_attributes_public,  file.path(path_outputs_national, purpose_public, geography, "d_all_attributes.csv"))
...
write_csv(lad_attributes,  file.path(path_outputs_national, purpose_public, "lad_attributes.csv"))
