...
#Progress Bar
pb <- winProgressBar(title="Renaming progress bar", min=0, max=nrow(lines), initial=0, label="0 lines done")
progress = 0

for(numb in 100000:200000){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
  progress <- progress + 1
  info <- sprintf("%d lines done", progress)
  setWinProgressBar(pb, progress, label = info)
  }
close(pb)
saveRDS(lines,paste0(path,line_file,"_updatedMSOA.Rds"))


for(numb in 200000:300000){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
}
saveRDS(lines,paste0(path,line_file,"_updatedMSOA_2-3.Rds"))
print("2-3 done")
print(Sys.time())


for(numb in 300000:400000){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
}
saveRDS(lines,paste0(path,line_file,"_updatedMSOA_3-4.Rds"))
print("3-4 done")
print(Sys.time())

for(numb in 400000:500000){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
}
saveRDS(lines,paste0(path,line_file,"_updatedMSOA_4-5.Rds"))
print("4-5 done")
print(Sys.time())

for(numb in 500000:600000){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
}
saveRDS(lines,paste0(path,line_file,"_updatedMSOA_5-6.Rds"))
print("5-6 done")
print(Sys.time())

for(numb in 600000:700000){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
}
saveRDS(lines,paste0(path,line_file,"_updatedMSOA_6-7.Rds"))
print("6-7 done")
print(Sys.time())

for(numb in 700000:843517){
  lines@data$msoa1C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[1,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[1,2] == lookup$coords.x2)])
  lines@data$msoa2C[numb] <- as.character(lookup$geo_code[which(lines@lines[[numb]]@Lines[[1]]@coords[2,1] == lookup$coords.x1 & lines@lines[[numb]]@Lines[[1]]@coords[2,2] == lookup$coords.x2)])
}
saveRDS(lines,paste0(path,line_file,"_updatedMSOA_7-all.Rds"))
print("7-all done")
print(Sys.time())
