...
levels(dfsp$scenario)[1:6] <- c("Total no. commuters", "Cyclists in Ebikes", "Cyclists in Go Dutch",  "Cyclists in Government Target (equity)", "Cyclists in Government Target (near market)", "Cyclists in Gender Equality", "Cyclists in Census 2011")
...
Finally, the **Route Network (LSOA, clickable)** and the **Route Network (LSOA, image)** are  built from all between-zone LSOA fast routes (including any that start *or* end in `r get_pretty_region_name(region)`). `r if(region_build_param$mingodutch_rnet==0){"The online version of the clickable route network and the downloads both include all route segments."}` `r if(region_build_param$mingodutch_rnet!=0){paste0("In the online version of the clickable route network we only present route segments that contain at least", `r region_build_param$mingodutch_rnet cyclists`, " in the Go Dutch scenario. The downloads include all route segments."}` For more details please see User Manual C.
