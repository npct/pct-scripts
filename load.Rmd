...
source(file.path("components", "set-up.R")) # pull in packages needed
```
...
if(!file.exists(pct_privatedata)) warning(paste("The pct_privatedata repository has cannot be found in", dirname(getwd())))
cyclestreet_pat() # Check user has cyclestreets API key
```
...
flow <- readRDS(file.path(pct_bigdata, "flow.Rds"))

if(file.exists(pct_privatedata)) flow <- readRDS(file.path(pct_privatedate, "flowsex-merged.Rds"))
...
if(file.exists(file.path("pct-data", region, "rf.Rds")) &
   file.exists(file.path("pct-data", region, "rq.Rds"))){
  rf <- readRDS(file.path("pct-data", region, "rf.Rds")) # if you've loaded them
  rq <- readRDS(file.path("pct-data", region, "rq.Rds"))
  ...
  saveRDS(rf, file.path("pct-data", region, "rf.Rds")) # save the routes
  saveRDS(rq, file.path("pct-data", region, "rq.Rds"))
}
...
```{r, echo=FALSE}
source(file.path("components", "model", "aggregate.R")) # this model creates the variable 'slc'
cormod <- cor(flow$clc, mod_logsqr$fitted.values) # crude indication of goodness-of-fit
# summary(mod_logsqr)

mod_nat <- readRDS(file.path(pct-bigdata, "mod_logsqr_national_8.Rds"))
```
...
if(exists("isolated")) file.create(file.path("pct-data", region, "isolated"))
saveRDS(zones, file.path("pct-data", region, "z.Rds"))
saveRDS(cents, file.path("pct-data", region, "c.Rds"))
saveRDS(l, file.path("pct-data", region, "l.Rds"))
saveRDS(rf, file.path("pct-data", region, "rf.Rds"))
saveRDS(rq, file.path("pct-data", region, "rq.Rds"))
saveRDS(rnet, file.path("pct-data", region, "rnet.Rds"))
saveRDS(mod_logsqr, file.path("pct-data", region, "model.Rds"))

# 
# # Save the script that loaded the lines into the data directory
file.copy("load.Rmd", file.path("pct-data", region, "load.Rmd"))

# Create folder in shiny app folder (assuming it's next to pct)
dir.create(file.path("..", "pct-shiny", region))
file.copy("../pct-shiny/leeds/ui.R", file.path("..", "pct-shiny", region, "ui.R"))
server_text <- readLines("../pct-shiny/leeds/server.R")
server_text <- gsub(pattern = "leeds", replacement = region, x = server_text)
writeLines(server_text, con = file.path("..", "pct-shiny", region, "server.R"))
```
