---
title: "PCT - local LSOA commute results"
...
### Key information for `r get_pretty_region_name(region)`, commute layer, LSOA geography
...
This layer of the PCT uses origin-destination data on travel to work from the 2011 Census. The interface is currently displaying the number of commuters travelling by different modes between Lower Super Output Area zones (LSOAs, average size 690 commuters). There were `r prettyNum(sum(z$all), big.mark = ",", scientific = F)` commuters living in `r get_pretty_region_name(region)` recorded in the 2011 Census. All of these commuters are included in the **zones** data that are visualised on the map interface and available for download.  

Adding in inter-regional flows, there were `r prettyNum(sum(od_attributes$all), big.mark = ",", scientific = F)` commuters living or working in `r get_pretty_region_name(region)` in the 2011 Census, and commuting in a flow containing at least 3 commuters (in either direction).  These commuters are included in the **All flows** files on the data download pages. Flows containing fewer than 3 commuters are excluded from the visualisation and download files for data protection reasons.

**Straight lines**, **Fast routes** and **Quieter routes** are both visualised on the map interface and available to download.  In the downloads, these lines and routes contain all `r prettyNum(region_build_param$n_flow, big.mark = ",", scientific = F)` flows between LSOAs that a) start or end in `r get_pretty_region_name(region)`, b) have a fast route distance <10km, and c) contain at least `r region_build_param$minflow_visualise` commuters (by any mode,  in either direction). These `r prettyNum(region_build_param$n_flow, big.mark = ",", scientific = F)` between-zone LSOA flows contain `r prettyNum(region_build_param$n_people, big.mark = ",", scientific = F)` commuters who either live or work in `r get_pretty_region_name(region)`. To speed up load times, the online map only shows those lines and routes with a fast route distance <10km.

Between-zone LSOA flows exclude **within-LSOA travel**, when the LSOA zone of origin is the same as the zone of destination. Within-LSOA travel is represented by red points on the map when the lines are shown, and accounts for `r prettyNum(sum(c$all), big.mark = ",", scientific = F)` commuters living in `r get_pretty_region_name(region)`.  Between-zone flows also exclude those with no fixed place of work.
...
`r rnet_txt`
 For more details please see User Manual C1.
...
Figure 1 shows a map of `r get_pretty_region_name(region)`, including the regional boundary (thick black border), local authority boundaries (blue lines) and LSOA zones in the region (grey with a black border). The straight green lines represent the top 100 flows in terms of the number of commuters by any mode.
...
*Figure 2: Map showing a) the proportion of commuters with a fast route commute distance <10km (left) and b) the average hilliness of commute trips <10km (right), in LSOA zones across the study area*
...
Table 1 summarises the proportion of commuters cycling, walking, driving and using other modes under each scenario in `r get_pretty_region_name(region)`, based on aggregating across all residents in all the LSOA zones.
...
| Scenario | Comparison	| Change in YLLs/year  | Change in years sick leave/year | Value of YLL+sick leave changes (million pounds)  | Change in tonnes CO2e/year |
| -------- |:----------:|:-----------------: |:----------:| :----------:| :---------------:|
| Census 2011 |	'No Cyclists' |  `r (sprintf('%.1f', round(sum(z$base_slyll) , 1)))` |  `r (sprintf('%.1f', round((sum(z$base_slsickdays) / 365.25), 1)))` | `r (sprintf('%.2f', round(sum(z$base_slvaluecomb) / 1000000  , 2)))` | `r (round(sum(z$base_slco2) / 1000))` |
| Government Target (equality)   |	Census 2011 |  `r (sprintf('%.1f', round(sum(z$govtarget_siyll) , 1)))` | `r (sprintf('%.1f', round((sum(z$govtarget_sisickdays) / 365.25), 1)))` |`r (sprintf('%.2f', round(sum(z$govtarget_sivaluecomb) / 1000000  , 2)))` | `r (round( sum(z$govtarget_sico2) / 1000))` |
| Government Target (near market) |	Census 2011 |  `r (sprintf('%.1f', round(sum(z$govnearmkt_siyll) , 1)))` | `r (sprintf('%.1f', round((sum(z$govnearmkt_sisickdays) / 365.25), 1)))` | `r (sprintf('%.2f', round(sum(z$govnearmkt_sivaluecomb) / 1000000  , 2)))` | `r (round( sum(z$govnearmkt_sico2) / 1000))` |
| Gender Equality |	Census 2011 |  `r (sprintf('%.1f', round(sum(z$gendereq_siyll) , 1)))` | `r (sprintf('%.1f', round((sum(z$gendereq_sisickdays) / 365.25), 1)))` | `r (sprintf('%.2f', round(sum(z$gendereq_sivaluecomb) / 1000000  , 2)))` | `r (round( sum(z$gendereq_sico2) / 1000))` |
| Go Dutch |	Census 2011 |  `r (sprintf('%.1f', round(sum(z$dutch_siyll) , 1)))` | `r (sprintf('%.1f', round((sum(z$dutch_sisickdays) / 365.25), 1)))` | `r (sprintf('%.2f', round(sum(z$dutch_sivaluecomb) / 1000000  , 2)))` | `r (round( sum(z$dutch_sico2) / 1000))` |
| Ebikes  |	Census 2011 |  `r (sprintf('%.1f', round(sum(z$ebike_siyll) , 1)))` | `r (sprintf('%.1f', round(sum((z$ebike_sisickdays) / 365.25), 1)))` | `r (sprintf('%.2f', round(sum(z$ebike_sivaluecomb) / 1000000  , 2)))` | `r (round( sum(z$ebike_sico2) / 1000))` |
