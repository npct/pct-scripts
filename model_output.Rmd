...
load(file.path(pct_data, region, "params.RData"))
if(!exists("regions"))
  regions <- readOGR(file.path(pct_bigdata, "regions-london.geojson"), layer = "OGRGeoJSON")
...
Please include this paragraph in any paper or report produced using these model results.

Data for `r region` were generated at `r date()` by the National Propensity to
Cycle Tool (PCT) Version 1, developed by the PCT academic team and released with
an GPL Affero [Licence] (https://www.gnu.org/licenses/agpl-3.0). This version
examines cycling potential for main-mode commute trips using Census 2011 travel
to work data and Middle-Level Super Output areas (MSOASs) as zones. There were
<!-- PLS ADD NUMBER COMMUTERS--> commuters in the study area, all of which were
included in area-based analysis. Where cycle flows are routed, flows within
zones ( <!--PLS add % of all flows that are within-zone--> of flows in `r region`
) are excluded as are commuters with no fixed workplace. Of the `r n_flow_region`
...
selected, accounting for ( <!--PLS add % of all commutes included in area-based
data that this represents--> of total commutes in `r region`). A modified
version of the 2014 [HEAT tool]
(http://www.euro.who.int/en/health-topics/environment-and-health/Transport-and-health/publications/2011/health-economic-assessment-tools-heat-for-walking-and-for-cycling.-methodology-and-user-guide.-economic-assessment-of-transport-infrastructure-and-policies.-2014-update)
was used.
