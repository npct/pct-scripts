...
Finally, the **Route Network (LSOA, clickable)** is built from all origin-destination fast routes within the region that contain at least `r region_build_param$minflow_rnet` children and have a fast route distance <5km (for primary school children)/<10km (for secondary school children). The pop-ups do not present exact numbers for segments containing 1 to 2 cyclists.
...
`r round(100 * (sum(z$govequity_slc) - sum(z$bicycle)) / sum(z$all) , 1)`%, based on the distance and hilliness of school trips in the region using the baseline model for England.
...
| Government Target (equity) | `r paste(sprintf('%.1f', (b <- round(100 * sum(z$govequity_slc) / sum(z$all) , 1))), '%')` | `r paste(sprintf('%.1f', (f <- round(100 * sum(z$govequity_slw) / sum(z$all) , 1))), '%')`|`r paste(sprintf('%.1f', (cd <- round(100 * sum(z$govequity_sld) / sum(z$all) , 1))), '%')` | `r paste(sprintf('%.1f', (100 - b - f - cd)), '%')` |
