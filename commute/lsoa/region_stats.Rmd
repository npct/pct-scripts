...
**Straight lines**, **Fast routes** and **Quieter routes** are both visualised on the map interface and available to download.  These lines and routes contain all `r prettyNum(region_build_param$n_flow, big.mark = ",", scientific = F)` flows between LSOAs that a) start or end in `r get_pretty_region_name(region)`, b) have a fast route distance <10km, and c) contain at least `r region_build_param$minflow_visualise` commuters (by any mode,  in either direction). These `r prettyNum(region_build_param$n_flow, big.mark = ",", scientific = F)` between-zone LSOA flows contain `r prettyNum(region_build_param$n_people, big.mark = ",", scientific = F)` commuters who either live or work in `r get_pretty_region_name(region)`. 
